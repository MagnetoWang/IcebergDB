...
//     table_name = table_name + std::to_string(ibdb::base::GetSecondTimestamp());
//     Schema schema;
...
//     bool ok = table.Sync();
//     assert(ok);
//     std::sort(running_time.begin(), running_time.end());
//     uint32_t size = running_time.size();
//     std::cout << "Percentile:99=" << running_time[(uint32_t)(size * 0.99)] 
//               << " ,95=" << running_time[(uint32_t)(size * 0.95)]
//               << " ,90=" << running_time[(uint32_t)(size * 0.90)]
//               << " ,50=" << running_time[(uint32_t)(size * 0.5)]
//               << std::endl;
...
//             LOG(ERROR) << "get the sum of running time is " << (uint64_t)std::accumulate(running_time.begin() , running_time.end() , 0);
//         }
//         ASSERT_EQ(result, true);
...
//     LOG(ERROR) << "In cache get the sum of running time is " << (uint64_t)std::accumulate(running_time.begin() , running_time.end() , 0);
// }

TEST(TablePerformance, PutDeepLearning) {
    std::string table_name("PutPerformances");
    table_name = table_name + std::to_string(ibdb::base::GetSecondTimestamp());
    Schema schema;

    Field* data = schema.add_field();
    Field* timestamp = schema.add_field(); // actually it is sample
    Field* feature = schema.add_field();
    Field* row = schema.add_field();


    feature->set_name("feature");
    feature->set_type("string");
    feature->set_is_key(false);
    data->set_name("data");
    data->set_type("string");
    data->set_is_key(false);
    timestamp->set_name("timestamp");//row
    timestamp->set_type("uint64_t");
    timestamp->set_is_key(false);
    row->set_name("row");//row
    row->set_type("uint64_t");
    row->set_is_key(true);
    Table table(table_name, schema);
    ...
    std::vector<std::string> insert_vec;
    insert_vec.reserve(number);
    insert = insert + " " + table_name + " feature,timestamp,data,row ";
    std::vector<uint64_t> running_time;
    std::string data_value(10000, 'a');
    LOG(ERROR) << "sample = " << number;
    LOG(ERROR) << "data_value = " << data_value.size();
 
    for (int i = 0; i < number; i++) {
        std::string feature_str = "feature";
        std::string timestamp_str(std::to_string(i));
        std::string tmp_value = data_value + timestamp_str;
        
        std::string statement;
        statement = insert + feature_str + "," + timestamp_str + "," + tmp_value + "," + timestamp_str;
        // LOG(ERROR) << statement;
        // LOG(ERROR) << statement;
        uint64_t start_timestamp = ibdb::base::GetMillisecondTimestamp();
        bool result = table.Put(statement);
    ...
    std::string table_get("get");
    table_get = table_get + " " + table_name + " row ";
    for (int i = number -1 ; i >= 0; i--) {
        std::string row_str(std::to_string(i));
        std::string timestamp_str(std::to_string(i));
        std::string statement;
        statement = table_get + row_str + " " + timestamp_str;
        std::string message;
