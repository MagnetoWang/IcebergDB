...
```flow
st=>start: Table
put=>operation: Put Data
pack=>operation: Get Offset and Data Size
tolog=>operation: Write to Log

logindex=>condition: Write to Index Log?

getPos=>operation: Get Physical Position
toindex=>operation: Write to Index Log
tomem=>operation: Write to memory
e=>end: End

st->put->pack->tolog->logindex
logindex(yes)->getPos->toindex->tomem
logindex(no)->tomem
tomem->e
```
...
```mermaid
graph TD
insert[insert table_name key,key,key value,value,value]

diskLog{offset, size, message}
diskPos[offset, pos]
memIndex[key, timestamp, offset]
...
```
##### 接口设计-参考elasticsearch


- putdisk statement
- putindex key, timestamp, offset
...
- schema 不支持 json格式
- 跳表支持对象，不支持基本类型
- 跳表模板在支持指针和对象有冲突，尤其是返回value函数中有问题